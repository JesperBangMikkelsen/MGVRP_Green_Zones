...
////Graph drawer
#include "Graph_drawer/DRAWGRAPH.h"
...
////Structure for cuts (Tournament, TSP, etc.)
//typedef struct {
//	int ConsType;
//	int IntListSize;
//	int* IntList; //Contains original set of nodes
//	int* ExtList; //Contains nodes for strengthening the inequality
//	double RHS;
//	double Coeff;
//} EnergyCnstrRecord;
//
//typedef EnergyCnstrRecord *EnergyCnstrPointer;
//typedef EnergyCnstrPointer *EnergyCnstrList;
//
//typedef struct {
//	EnergyCnstrList Cuts;
//	int Size;
//	int Dim;
//}EnergyCnstrMgr;
//typedef EnergyCnstrMgr* EnergyCnstrMgrPointer;
//
...
public:
	...
	struct connections {
		std::vector<int> succ;
		std::vector<int> pre;
		std::vector<double> succVals;
		std::vector<double> preVals;
		std::vector<int> preCharger;
		std::vector<int> succCharger;
	};

	struct UsedEdges {
		std::vector<connections> DirectConnections;
		std::vector<connections> ChargerConnections;
		std::vector<connections> ICEVConnections;
		std::vector<connections> Consolidated;

		UsedEdges(int n)
			: DirectConnections(n), ChargerConnections(n), ICEVConnections(n), Consolidated(n) {}
	};
	...
	//Public parameters:
	IloNum3DMatrix c; //Distance between node i and j
	IloNum2DMatrix ICEV_Cost_matrix;
	double GZPenalty;
	...
	std::vector<std::vector<double>> GreenZones;
	//Constraints
	IloRangeArray cons; //Holds all initial constraints
	std::vector<std::string>consDesc;
	bool runRefiner;
	//Fractional Cuts
	...
	//RESTRUCT FUNCTIONS:
		//Cut separation procedures:
	void SeparateCuts(UsedEdges* edges, CnstrMgrPointer MyCuts);
	void FinalFeasibilityCheck(std::vector<connections>* Direct, std::vector<connections>* Charger, std::vector<connections>* ICEV, CnstrMgrPointer MyCuts);
	void SeparateEnergyCuts(std::vector<connections>* Direct, std::vector<connections>* Charger, CnstrMgrPointer MyCuts);
	void SeparateDurationCuts(std::vector<connections>* ICEV, std::vector<connections>* Direct, std::vector<connections>* Charger, std::vector<connections>* Consolidated, CnstrMgrPointer MyCuts);
	void ConnectedComponents(UsedEdges* edges, CnstrMgrPointer MyCuts); //Connected components heuristic
	void DFSUtill(int v, std::vector<bool>& visited, std::vector<int>& component, double totalWeight, double consumEnergy, double travelTime, double serviceTime, bool energy, bool ICEV, std::vector<connections>* graph, CnstrMgrPointer MyCuts); //Depth first search for connected components heuristic
	//void DFSUtillChargers(int v, std::vector<bool>& visited, std::vector<int>& cutSet, std::vector<int>& chargers, double totalWeight, double travelTime, double serviceTime, int chargerAdded, std::vector<connections>* charging, std::vector<connections> *direct, CnstrMgrPointer MyCuts); //Depth first search for connected components heuristic
	void CheckChargerConnections(std::vector<connections>* Charger, connections* Depot, std::vector<int>* path, std::vector<std::pair<int, double>>& EligibleIngoingChargers, std::vector<std::pair<int, double>>& EligibleOutgoingChargers, std::vector<std::pair<int, double>>& EligibleIngoingChargersSet, std::vector<std::pair<int, double>>& EligibleOutgoingChargersSet);
	double TSPConcorde(std::vector<int>* path, int VehicleIndex=0, bool Duration = false, bool symmetric = false, bool fixed = false, int charg_in = -1, int charg_out = -1);
	void setupTSPEnergyMatrix(std::vector<int>* path, std::vector<std::vector<double>>& dist, bool symmetric, bool fixed = false, int charg_in = -1, int charg_out = -1);
	void setupTSPDurationMatrix(std::vector<int>* path, std::vector<std::vector<double>>& dist, int VehicleIndex);

	void solveTSPConcorde(int* elist, int* elen, int nCities, int nEdges, double& optVal);
	void AddNoChargeSet(std::vector<int>* p, CnstrMgrPointer MyCuts);
	void AddNoChargePath(std::vector<int>* p, CnstrMgrPointer MyCuts);
	void AddNoChargeTour(std::vector<int>* p, CnstrMgrPointer MyCuts);
	void AddChargerTour(std::vector<int>* p, int in_charger, int out_charger, CnstrMgrPointer MyCuts);
	void AddFixedCharger(std::vector<int>* p, int in_charger, int out_charger, CnstrMgrPointer MyCuts);
	void AddFixedChargingEdge(std::vector<int>* p, int in_charger, int out_charger, CnstrMgrPointer MyCuts);
	void AddDurationConstraint(std::vector<int>* p, int ICEV, CnstrMgrPointer MyCuts);
	void AddDurationTour(std::vector<int>* p, int ICEV, CnstrMgrPointer MyCuts);
	void AddDurationSet(std::vector<int>* p, int ICEV, CnstrMgrPointer MyCuts);

	// Setup cut in LP form:
	void NoChargeTourExpr(IloExpr& expr, CnstrPointer cut);
	void NoChargePathExpr(IloExpr& expr, CnstrPointer cut);
	void NoChargeSetExpr(IloExpr& expr, CnstrPointer cut);
	void ChargerTourExpr(IloExpr& expr, int chargerInfo, CnstrPointer cut);
	void FixedChargerExpr(IloExpr& expr, int chargerInfo, CnstrPointer cut);
	void FixedChargingEdgeExpr(IloExpr& expr, int chargerInfo, CnstrPointer cut);
	void DurationTourExpr(IloExpr& expr, int ICEV, CnstrPointer cut);
	void DurationSetExpr(IloExpr& expr, int ICEV, CnstrPointer cut);
	void MTSTARExpr(IloExpr& expr, CnstrPointer cut);
	void CAPExpr(IloExpr& expr, CnstrPointer cut);
	void FCIExpr(IloExpr& expr, CnstrPointer cut);
	void COMBExpr(IloExpr& expr, CnstrPointer cut);
	...
	double getServiceTime(int idx) { return s[idx]; }
